...
const Products_Computer = [
    ...
    },
    {
        ...
        "products": [
            ...
            { "name": "Samsung Odyssey OLED", "spec": 'G6 G60SD (Pivot) 27" / OLED', "price": "910.00", "image": "https://www.goldonecomputer.com/image/cache/catalog/Products/Monitor%20Samsung/G6-200x241.jpg" },
    ...
    if (cartItems.length === 0) {
        ...
        cartItemsContainer.innerHTML = cartItems.map((item, index) => `
            <div class="flex items-center space-x-4 p-4 bg-gray-100 rounded-lg">
                ...
                <button class="remove-item p-2 bg-red-500 text-white rounded-md hover:bg-red-600" data-index="${index}">Cancel</button>
            </div>
        `).join('');
...
function renderViewProduct(product) {
    ...
    if (viewProductContainer) {
        viewProductContainer.innerHTML = `
            <img src="${product.image}" alt="${product.name}" class="w-full max-w-xs mx-auto rounded-md">
    ...
    category_computer.innerHTML = Products_Computer.map(category => {
        ...
        return `
            <div class="mt-6">
                ...
                <button class="mt-3 px-4 py-2 bg-yellow-500 text-white rounded-md hover:scale-105 transition-all duration-300">VIEW ALL</button>
                <button class="mt-3 px-4 py-2 bg-green-500 text-white rounded-md hover:scale-105 transition-all duration-300 new-cart-btn" data-category='${category.category}'>New Cart</button>
            </div>
        `;
    }).join('');
